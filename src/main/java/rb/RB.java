...
public final class RB {
  ...
  public int run() throws IOException, FailedGenerationException {
    ...
    for (long seed = nbInstances; --seed >= 0;) {
      ...
      final CSPOMSolver solver = Solver.apply(rb.generate());
      if (!solver.hasNext()) {
        unsat++;
      }
    }
  ...
  public int runFilter(Class<? extends Filter> clazz) throws IOException {
    ...
    for (long seed = nbInstances; --seed >= 0;) {
      ...
      try {
        problem = pg.generate(rb.generate())._1;
   ...
   */
  public static void main(String[] args) throws NumberFormatException, FailedGenerationException {
