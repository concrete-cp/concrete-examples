...
public class RBGenerator {
	...
	public CSPOM generate() throws FailedGenerationException {
		...
		for (int i = 0; i < activeConstraints.length; i++) {
			...
			cspom.ctr(buildExplicitConstraint(involvedVariables, tightnessMode,
					tightness, RAND.nextLong(), incompatibilityGraphType,
					forcedTuple));
		}
	...
	private Map<IntVariable, Integer> computeRandomSolution(
		...
		for (IntVariable v : variables) {
			ContiguousIntRangeSet dom = new ContiguousIntRangeSet(v.domain());
			solution.put(v, RAND.nextInt(dom.size()));
		}
		return solution;
	}
	...
	private long computeNbUnallowedTuplesFrom(IntVariable[] variables,
		...
		for (int i = variables.length; --i >= 0;) {
			ContiguousIntRangeSet dom = new ContiguousIntRangeSet(
					variables[i].domain());
			cpt *= dom.size();
		}
	...
	private CSPOMConstraint<?> buildExplicitConstraint(IntVariable[] variables,
		...
		return constraint(
				"extension",
				variables,
				CSPOMConstraint.param("relation", matrix.e).param("init",
						matrix.sup));
	}
