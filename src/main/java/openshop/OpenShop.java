...
public class OpenShop {
	public static void main(final String[] args) throws IOException, FailedGenerationException {
		...
		case 1:
			generator = OpenShopGenerator.apply(cl.getArgs()[0]);
			break;

		case 3:
			generator = OpenShopGenerator.apply(Integer.valueOf(cl.getArgs()[0]), Integer.valueOf(cl
					.getArgs()[1]), Integer.valueOf(cl.getArgs()[2]), cl.hasOption("js"));
			break;
		...
		while (ub > lb) {
			...
			if (!solution.isSat()) {
				...
				final Set<CSPOMConstraint> control = cspom.control(solution.getNum());
				if (control.size() > 0) {
