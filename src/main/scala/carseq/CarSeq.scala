...
object CarSeq extends ConcreteRunner with App {
  ...
  override def loadCSPOM(args: List[String]) = {
    ...
    CSPOM {
      val (cars, cn) = (0 until nbCars).map(c => IntVariable.ofInterval(0, nbClasses - 1) withName s"car$c").unzip

      carNames = cn
      val oc = cars.zipWithIndex map {
        case (cv, c) =>
          val vars = (0 until nbOptions) map (o => IntVariable.of(0, 1) withName s"car${c}option$o")
  ...
  def sequence(cp: CSPOM, vars: IndexedSeq[IntVariable], u: Int, q: Int, cardinality: Int) {
    for (i <- 0 to vars.size - q) {
      val ub = IntVariable.ofInterval(-u, 0)
