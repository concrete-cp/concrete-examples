 ...
 * Tuples are selected by managing arrays containing indexes coresponding to tuples.
 * Indeed, using the number of values as a base, it is possible to associate an index with each tuple.
 * This way of generating random lists can only be performed with a limited number of potential tuples.
  */
public class FineProportionRandomListGenerator extends ProportionRandomListGenerator
{
	...
	private void fillIndexStructures()
	{
		...
		for (int i = 0; i < validIndexes.length; i++)
			validIndexes[i] = (valueRepetition ? i : BasisConvertor.getDecimalValueFor(iterator.next(), nbValues));
	}
	...
	private void manageRequiredElement()
	{
		if (fixedTuple != null)
		{
			int position = Arrays.binarySearch(validIndexes, BasisConvertor.getDecimalValueFor(fixedTuple, nbValues));
		...
		for (int i = (fixedTuple != null && requiredFixedTuple ? 1 : 0); i < tuples.length; i++)
		{
			...
			while (!found)
			{
				...
				while (!found && nbValidIndexes > 0)
				{
					...
					tuples[i] = BasisConvertor.getValueFor(index, tuples[i], nbValues);
